  ...
  return {
    ...
    getFade: () => {
      return statState === "dying" || statState === "killed"
        ? 1 - Math.min(1, (getNow() - statTime) / 300)
        : 1;
    },
    getColor: () => {
      return "#FFFFFF";
    ...
    getBlend: () => {
      let o =
        statState === "normal" || statState === "dying"
          ? 0
          : statState === "invuln"
          ? 0.125 + Math.sin((getNow() - statTime) / 33) / 8
...
const socketInit = (port) => {
  ...
  socket.onopen = function socketOpen() {
    socket.open = true;
    global.message = "Creating a connection to the server...";
  ...
  socket.onmessage = async function socketMessage(message) {
    ...
    switch (m.shift()) {
      ...
      case "S": // clock syncing
        ...
        if (sync.length < 32&& !global.syncInterval) {
