...
const util = {
    ...
    pullJSON: fileName => {
        return new Promise((resolve, reject) => {
            let url = `${location.protocol}//${window.serverAdd}/lib/json/${fileName}.json`;
            if (window.serverAdd.includes("localhost")) {
              url = `http://${window.serverAdd}/lib/json/${fileName}.json`;
            }
